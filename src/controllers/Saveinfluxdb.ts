...
// export const getSewageLevels = async (req: Request, res: Response): Promise<void> => {
//     try {
//         const { stationId, bayId } = req.query;

//         let fluxQuery = `
//             from(bucket: "${bucket}")
//             |> range(start: -24h) 
//             |> filter(fn: (r) => r._measurement == "sewage_levels")
//         `;

//         if (stationId) {
//             fluxQuery += `|> filter(fn: (r) => r.station == "${stationId}")`;
//         }
//         if (bayId) {
//             fluxQuery += `|> filter(fn: (r) => r.bay == "${bayId}")`;
//         }

//         const sewageData: any[] = [];

//         queryApi.queryRows(fluxQuery, {
//             next(row, tableMeta) {
//                 sewageData.push(tableMeta.toObject(row));
//             },
//             error(error) {
//                 console.error(`Query error: ${error.message}`);
//                 res.status(500).json({ error: "query_failed", message: "Failed to fetch sewage data." });
//             },
//             complete() {
//                 res.status(200).json(sewageData);
//             }
//         });
//     } catch (err: any) {
//         console.error(`Error fetching sewage levels: ${err.message}`);
//         res.status(500).json({ error: "unexpected_error", message: "An error occurred while retrieving sewage levels." });
//     }
// };