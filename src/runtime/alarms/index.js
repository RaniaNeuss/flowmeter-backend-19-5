...
// It checks the values of monitored variables (like temperature, pressure, etc.) to see if they exceed predefined thresholds.
// If the values go out of range, an alarm is triggered.
// Alarm Management:

// Keeps track of active alarms and their states (e.g., active, acknowledged, cleared).
// Saves alarms and their statuses in a database for persistence.
// Notification and Actions:

// Notifies other parts of the system (e.g., the user interface or scripts) about changes in alarm states.
// Executes predefined actions when alarms are triggered, such as running a script, setting a variable, or showing a popup.
// Alarm History:

// Maintains a log of all past alarms for auditing or analysis.
// Provides APIs to retrieve current alarms or historical alarm data.
// Permissions:

// Checks if a user has the proper permissions to acknowledge or interact with alarms.
// State Management:

// Operates as a state machine with states like:
// INIT: Preparing resources (e.g., initializing the database).
// LOAD: Loading alarm configurations and current statuses.
// IDLE: Continuously monitoring and processing alarms.
// Relation to the Project
// Integration with Devices:

// It interacts with the devices in the system to fetch data (e.g., temperature, pressure) and detect alarms.
// It uses runtime.devices to retrieve values from sensors or connected hardware.
// Runtime Communication:

// It uses the event system (runtime.events) to inform other parts of the project (e.g., user interface or scripts) about alarm changes.
// Database Interaction:

// It relies on a database (e.g., alarms.fuxap.db) to persist alarm data so that alarms aren't lost when the system restarts.
// User Interface Support:

// Provides real-time alarm data to the frontend or dashboard.
// It ensures users can see active alarms, their details, and acknowledge them if needed.
// Script Execution:

// Executes automation tasks based on alarm conditions (e.g., running scripts to mitigate an issue automatically).
// User and Security:

// Enforces user permissions for managing alarms, ensuring only authorized users can interact with them.
// In the Context of the Project
// This code is critical for managing faults, warnings, and events within the system.
// It ensures reliability by continuously monitoring the system and informing relevant components or users of potential issues.
// By linking to other parts of the project (e.g., devices, database, scripts, user interface), it acts as the backbone for real-time alarm management.
'use strict';
        ...
        Object.keys(alarms).forEach(alrkey => {
            alarms[alrkey].forEach(alr => {
                if (alr.status && alr.type !== AlarmsTypes.ACTION) {
                    // Parse `subproperty` from JSON string
                    const subproperty = alr.subproperty
                        ? JSON.parse(alr.subproperty)
                        : {};
    
                    // Build the alarm item with parsed `subproperty`
                    var alritem = {
                        name: alr.getId(),
                        type: alr.type,
                        ontime: alr.ontime,
                        ...
                        group: subproperty.group || '',
                        bkcolor: subproperty.bkcolor || '',
                        color: subproperty.color || '',
                        toack: alr.isToAck()
                    };
    
                    // Check permissions if `tagproperty` is defined
                    var alrPermission = { show: true, enabled: true };
                    if (alr.tagproperty) {
                        alrPermission = runtime.checkPermission(permission, alr.tagproperty);
                    }
    
                    // Filter alarms and apply permissions
                    if (alrPermission.show && (!filter || _filterAlarm(
                        alr.type,
                        subproperty.text,
                        subproperty.group,
                        alr.tagproperty?.variableId,
                        filter
                    ))) {
